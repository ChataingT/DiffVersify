...
def load_1000_genomes_dataset(root='../data'):
    ...
    colpath = os.path.join(root, "1kgenomes_variants_af0.01_autosome_genebodyOnly.colnames")
    rowpath = os.path.join(root, "1kgenomes_variants_af0.01_autosome_promoterOnly.rownames")
    datapath = os.path.join(root, "1kgenomes_variants_af0.01_autosome_genebodyOnly.dat")
    labelpath = os.path.join(root, "20130606_sample_info.csv")
...
def load_1000_genomes(root):
    np.random.seed(None)
    st = np.random.get_state()[1][0]
    np.random.seed(2147483648)
    data, labels, super_labels, rev_label_dict, super_labels_translator, super_label_dict, columns = load_1000_genomes_dataset(root)
...
def load_cardio_vasc(root):
    ...
    """
    path = os.path.join(root, "cardio_train.csv")
...
def load_disease_symptom_prediction(root="../data/"):
    path = os.path.join(root,"disease_symptom.csv")
    symptom = pd.read_csv(path)
...
def load_brca_mult(root='../data'):

    path_data = os.path.join(root, "brca_processed_logtpm_tumor_noduplicates_75_perc_subtypebalanced.tsv")
    path_flags = os.path.join(root, "brca_subtype_flag.tsv")   
...
def load_brca_bin(root='../data'):
    path_data = os.path.join(root, "brca_processed_logtpm_balanced_75perc.tsv")
    path_flags = os.path.join(root, "brca_processed_logtpm_balanced_flag_tissue_origin.tsv")
    data = pd.read_csv(path_data,sep="\t")
    translator = data.columns
    data = data.to_numpy().astype(np.float32)
