...
def mean_dfs(dfs, method, exp, output_dir=None):
    ##### old version
    # final_df = []
    ...
    #     file = r"../../results/synth_results/%s/%s.csv"%(method,exp)
    #     file = os.path.join(root, file)
    # else:
    #     file = os.path.join(output_dir, "synth_results/%s/%s.csv"%(method,exp))
        
    # final_df.to_csv(file,index=True)
    # return  final_df
    #######
    xp_col_dict = {
        'exp1_diffnaps':'nlines',
        'exp2_diffnaps':'kclasses',
        'exp3_diffnaps':'Anoise',
        'exp4_diffnaps':'Dnoise'
    }
    xp_col = xp_col_dict[exp]
    df = pd.DataFrame()
    for s in range(len(dfs)):
        dfs[s]['seed'] = s
        df = pd.concat([df, dfs[s]], ignore_index=True)

    dm = df.groupby(['method', xp_col]).mean().reset_index().sort_values(xp_col)
    dm = dm.drop(columns='seed')
    dm = dm.round(2)
    ds = df.groupby(['method', xp_col]).std().reset_index().sort_values(xp_col)
    ds = ds.drop(columns='seed')
    ds = ds.round(2)

    dr = dm[['method', xp_col]].copy()
    for c in dm.columns:
        if not(c == 'method' or c == xp_col):
            dr[c] = dm[c].astype(str) + '±' + ds[c].astype(str)

    if not output_dir:
        root = os.path.dirname(os.path.realpath(__file__))
        file = r"../../results/synth_results/%s/%s.xlsx"%(method,exp)
        file = os.path.join(root, file)
    else:
        file = os.path.join(output_dir, "synth_results/%s/%s.xlsx"%(method,exp))
        
    dr.to_excel(file,index=True)
    return  dr
...
def res_to_csv(method, dataname, res_dict, data,labels,label_dict,translator,verbose=False, output=None):
...
def write_time(method, dataname, time_taken,res_dict, output_dir=None):
    folder = os.path.join("../results/real_results/",method)
    if output_dir:
        folder=output_dir
